...
// Primary operations
int InitializeFS(int fsDesc, long nBlocks, int nInodes);
...
// Allocation
void AddFreeBlock(int fsDesc, long blockNumber); // initFS
void AddToFreeList(unsigned int block);
...
int MakeIndirectBlock(int fsDesc, int block_num);
        ...
        if (strcmp(pCommand, "initfs") == 0) {
            ...
            if (access(pFilePath, F_OK) != -1) { // if exists
                ...
                if (fsDesc == -1) {
                    ...
                    fsStatus = InitializeFS(fsDesc, nBlocks, nInodes);
                    if (fsStatus == 0)
...
int InitializeFS(int fsDesc, long nBlocks, int nInodes) {
    ...
    lseek(fsDesc, BLOCK_SIZE, SEEK_SET);
    writtenBytes = write(fsDesc, &superBlock, BLOCK_SIZE); // Note: sizeof(superBlock) <= BLOCK_SIZE

    // Write zeroes to all inodes/inodeblocks
    for (i = 0; i < BLOCK_SIZE; i++) // full block size 0 buffer
        buffer[i] = 0;
    for (i = 0; i < nInodeBlocks; i++)
        write(fsDesc, buffer, BLOCK_SIZE);

    int usedBlocks = 2 + nInodeBlocks;
    int unusedBlocks = nBlocks-usedBlocks;
    // Link datablocks
    LinkDataBlocks(nBlocks);

    // SUPER-BLOCK: FREE LIST
    for (i = 0; i < 250; i++)
        AddToFreeList(usedBlocks + i);
...
//write to inode and directory data block
    if (indirectfn_return > -1) {
        ...
        lseek(fsDesc, 2 * BLOCK_SIZE, 0);
        read(fsDesc, &iNode, INODE_SIZE);
        iNode.nlinks++;
...
void CopyOUT(const char *src, const char *targ) {
    ...
    for (addrcount = 0; addrcount <= 21; addrcount++) {
        if (found_dir != 1) {
            lseek(fsDesc, (iNode.addr[addrcount] * BLOCK_SIZE), 0);
            for (i = 0; i < 64; i++) {
                if (found_dir != 1) {
                    read(fsDesc, &dir, 16);
    ...
    lseek(fsDesc, (2 * BLOCK_SIZE + INODE_SIZE * src_inumber), 0);
    read(fsDesc, &iNode, 64);
    ...
    if (indirect ==
        0)                //check if it is a small file. indirect = 0 implies the function that makes indirect blocks was not called during CopyIN.
    {
        printf("file size = %d \n", iNode.size1);
    ...
    if (indirect ==
        1)            //check if it is a large file. indirect = 1 implies the function that makes indirect blocks was called during CopyIN.
    {
...
void MakeDirectory(char *filename, unsigned int newinode) {
    ...
    lseek(fsDesc, 2 * BLOCK_SIZE, 0);
    blocks_read = read(fsDesc, &iNode, 64);
...
void CreateRootDirectory() {
    ...
    lseek(fsDesc, datablock * BLOCK_SIZE, 0);

    //filling 1st entry with .
    if ((bytes_written = write(fsDesc, &fileName, 16)) < 16)
...
int WriteDirectory(INode rootinode, FileName fileName) {
    ...
    int i = 0;
    for (addrcount = 0; addrcount <= 21; addrcount++) {
        lseek(fsDesc, rootinode.addr[addrcount] * BLOCK_SIZE, 0);
        for (i = 0; i < 64; i++) {
            read(fsDesc, &dir, 16);
...
int MakeIndirectBlock(int fsDesc, int block_num) {
    ...
    int extfilesize = 22 *
                      BLOCK_SIZE;            //filesize is initialized to small file size since data would have been read upto this size.
    for (i = 0; i < 22; i++)
    ...
    i = 22;
    while (1) {
        if ((bytes_read = read(fsDesc, reader, BLOCK_SIZE)) != 0) {
...
void LinkDataBlocks(unsigned short total_blcks) {
    ...
    for (blockcounter = 0; blockcounter < no_chunks; blockcounter++) {
        chainarray[0] = 250; // index of the starting block of next chunk
...
//***************************************************************************
void AddFreeBlock(int fsDesc, long blockNumber) {
...
void ReadCharBlock(char *target, unsigned int blocknum) {
    ...
    if (blocknum > superBlock.isize + superBlock.fsize + 2)
        ...
        lseek(fsDesc, blocknum * BLOCK_SIZE, 0);
        read(fsDesc, target, BLOCK_SIZE);
    }
}
...
void WriteIntBlock(unsigned int *target, unsigned int blocknum) {
    ...
    if (blocknum >= superBlock.isize + superBlock.fsize + 2)
        ...
        lseek(fsDesc, blocknum * BLOCK_SIZE, 0);
        if ((bytes_written = write(fsDesc, target, BLOCK_SIZE)) < BLOCK_SIZE)
...
void WriteInode(INode inodeinstance, unsigned int inodenumber) {
    int bytes_written;
    lseek(fsDesc, 2 * BLOCK_SIZE + inodenumber * INODE_SIZE, 0);
    if ((bytes_written = write(fsDesc, &inodeinstance, INODE_SIZE)) < INODE_SIZE)
